...
bool collapse_causes_intersection(const Ring& ring, Vertex* A, Vertex* B, Vertex* C, Vertex* D, Vec2 E, const SpatialGrid* grid) {
    ...
    if (grid) {
        ...
        bool found = false;
        grid->query(ax0, ay0, ax1, ay1, [&](Vertex* u, int rid) {
            if (found) return;
            if (rid != ring.ring_id) return;
            if (u == A || u == B || u == C || u == D) return;
            Vertex* w = u->next;
            if (w == A || w == B || w == C || w == D) return;
            Vec2 pu = {u->x, u->y};
            Vec2 pw = {w->x, w->y};
            if (segments_intersect(vA, E, pu, pw)) { found = true; return; }
            if (segments_intersect(E, vD, pu, pw)) { found = true; return; }
        });
        return found;
    }
...
bool collapse_causes_cross_ring_intersection(const Polygon& poly, int ring_id, Vertex* A, Vertex* B, Vertex* C, Vertex* D, Vec2 E, const SpatialGrid* grid) {
    ...
    if (grid) {
        ...
        grid->query(ax0, ay0, ax1, ay1, [&](Vertex* u, int rid) {
            if (found) return;
            if (rid == ring_id) return;
            Vertex* w = u->next;
            Vec2 pu = {u->x, u->y};
            Vec2 pw = {w->x, w->y};
            if (segments_intersect_nontrivial(vA, E, pu, pw, false)) { found = true; return; }
            if (segments_intersect_nontrivial(E, vD, pu, pw, false)) { found = true; return; }
        });
        return found;
    }
